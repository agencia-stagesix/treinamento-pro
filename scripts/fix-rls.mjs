...
const tables = {
  perfis: [
    ...
    [
      "perfis_select_treinador_ve_alunos",
      "SELECT",
  ...
  biometria: [
    ...
    [
      "biometria_treinador_select",
      "SELECT",
      "USING (agente_id IN (SELECT id FROM perfis WHERE treinador_id = auth.uid()))",
    ],
  ],
  treinos_realizados: [
    ["treinos_agente_crud", "ALL", "USING (agente_id = auth.uid())"],
    [
      "treinos_treinador_select",
      "SELECT",
      "USING (agente_id IN (SELECT id FROM perfis WHERE treinador_id = auth.uid()))",
    ],
  ],
  ...
  videos_treino: [
    ...
    [
      "videos_treinador_select",
      "SELECT",
      "USING (agente_id IN (SELECT id FROM perfis WHERE treinador_id = auth.uid()))",
    ],
    [
      "videos_treinador_update_comentario",
      "UPDATE",
      "USING (agente_id IN (SELECT id FROM perfis WHERE treinador_id = auth.uid()))",
    ],
  ],
  ...
  suplementacao_log: [
    ...
    [
      "suplementacao_treinador_select",
      "SELECT",
      "USING (agente_id IN (SELECT id FROM perfis WHERE treinador_id = auth.uid()))",
    ],
  ],
